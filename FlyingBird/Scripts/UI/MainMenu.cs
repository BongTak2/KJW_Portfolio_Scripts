...
public class MainMenu : MonoBehaviour
{
    ...
    public static void Save()
    {
        for (int i = 0; i < 3; i++)
        {
            saveData.keyCode[i] = KeySetting.keyValues[(KeyAction)i];
        }
    ...
    private void Load()
    {
        ...
        if (KeySetting.keyValues.Count == 0)
        {
            for (int i = 0; i < 3; i++)
            {
                KeySetting.keyValues[(KeyAction)i] = saveData.keyCode[i];
            }
        }
